...
use pango::{FontDescription, Alignment, WrapMode};
...
fn main() {
    let (font_description, pdf_file_name) = if cfg!(target_os = "windows") {
        ("Arial Unicode MS 12", "win_first.pdf")
    } else {
        ("NotoSansTC-Regular 12", "ubuntu_first.pdf")
    };

    let surface = PdfSurface::new(A4.width, A4.height, pdf_file_name).unwrap();
    let cr = Context::new(&surface).unwrap();

    let text = "Hello, 世界, مرحبا, こんにちは, Xin Chào";
